...
class CartPole:
    ...
    def show_cart(self, state_tuple, pause_time, interactive=False):
        """
        ...
        """
        x, x_dot, theta, theta_dot = state_tuple
        state = self.get_state(state_tuple)
        ...
        ax.set_title('x: %.3f, $%s$: %.3f, $%s$: %.3f, $%s$: %.3f, state: %d'\
                                %(x, x_dot_str, x_dot, theta_str, theta, theta_dot_str, theta_dot, state))
        if interactive:
            plt.show()
