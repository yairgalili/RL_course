...
###### BEGIN YOUR CODE ######
V_all = np.random.uniform(low=0.0, high=0.10, size=NUM_STATES)
R_all = np.zeros(NUM_STATES)
# old_state, action, new_state
P_all = np.full((NUM_STATES, NUM_ACTIONS, NUM_STATES), 1.0 / NUM_STATES)
T_statistics = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))
R_statistics = np.zeros(NUM_STATES)
###### END YOUR CODE ######
...
# he
# re, and say the overall algorithm has converged.
def update_value_function(P, R, V_old, gamma):
    A = P * (R + gamma * V_old.reshape((1, 1, -1)))

    Tv = np.sum(A, axis=2)
    pi_s = np.argmax(Tv, axis=1).astype(int)
    
    V_new = np.take_along_axis(Tv, pi_s.reshape((-1, 1)), axis=1).squeeze()
    
    max_diff = np.abs(V_new - V_old).max()
    return V_new, max_diff, pi_s


def value_iteration(P, R, gamma, max_iter, TOLERANCE):
    """
    Inputs:
        ...
        TOLERANCE: tolerance
    Outputs:
        (idx_iter, value_function, policy)

    size(value_function) == (nS) and size(policy) == (nS) and idx_iter is the number of iterations
    """
    nS = P.shape[0]
    V_old = np.zeros(nS)
    V_new = np.zeros(nS)
    ...
    for idx_iter in range(max_iter):
        V_new, max_diff, pi_s = update_value_function(P, R, V_old, gamma)
        V_old = V_new

        if max_diff < TOLERANCE:
            break

    return idx_iter, V_new, pi_s
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # action = argmax_a {r(s,a) + gamma *sum_s' P(s'|s,a) * V(s')}
    action = np.argmax(R_all[state] + GAMMA * np.sum(P_all[state, :, :] * V_all, axis=1))
    ...
    T_statistics[state, action, new_state] += 1
    R_statistics[new_state] += R
    ...
    if new_state == NUM_STATES - 1:
        ...
        ###### BEGIN YOUR CODE ######
        num_visited = np.sum(T_statistics, axis=2)
        is_visited = num_visited>0
        P_all[is_visited, :] = T_statistics[is_visited, :] / num_visited[is_visited, np.newaxis]

        num_state_visited = np.sum(T_statistics, axis=(0, 1))
        R_all[num_state_visited>0] = R_statistics[num_state_visited>0] / num_state_visited[num_state_visited>0]

        # assert that the transition probabilities are valid
        assert(np.all(np.isclose(np.sum(P_all, axis=2), 1)))
        # assert that the reward for the terminal state is -1
        assert(R_all[162]==-1)
        # assert that the terminal state is the only state with a reward
        assert(np.where(R_all)[0]==162)
        ###### END YOUR CODE ######
        ...
        ###### BEGIN YOUR CODE ######
        # idx_iter, V_all, pi_s = value_iteration(P_all, R_all, GAMMA, MAX_ITERS_VALUE_ITERATION, TOLERANCE)
        V_all, max_diff, _ = update_value_function(P_all, R_all, V_all, GAMMA)
    ...
    if new_state == NUM_STATES - 1:
        num_failures += 1
        if display_started == 1:
            plt.savefig(f"Failure_{num_failures}.jpg")
            plt.close('all')
...
# plt.show()
plt.title(f"Learning curve - converged after {num_failures} failures")
